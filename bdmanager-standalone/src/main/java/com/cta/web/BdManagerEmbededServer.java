...
public abstract class BdManagerEmbededServer extends AbstractMain {
	...
	public static void main(String[] args) {
		...
		if(cmd.hasOption('h')) {
			...
			int port = getOptionValueInt(cmd, "p", System.getProperty("app.port") == null ? 8090 : Integer.parseInt(System.getProperty("app.port"))); // Hack for cloudbees
	...
	protected static void stopServer(Server server) {
		if(server != null) {
			try {
				server.stop();
			} catch (Exception e1) {
